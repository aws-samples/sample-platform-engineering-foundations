...
The `postgres-root-user-password` Secret holds the master password Crossplane passes to the RDS
instance. Nothing later in this flow needs to know the value - the composition references the
Secret by name - so generate a random one and never see it:

```bash
kubectl create secret generic postgres-root-user-password \
  --namespace crossplane-system \
  --from-literal=password="$(openssl rand -base64 24 | tr -d '/+=' | head -c 24)"
```

`kubectl create secret` base64-encodes the value for you, so there is no manifest to edit and
no plaintext left in a file. In production this belongs in AWS Secrets Manager, delivered to the
cluster by External Secrets rather than created by hand.
